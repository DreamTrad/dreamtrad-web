---
Solution de l’échappatoire du sas de décompression dans Virtue’s Last Reward.
---
# Échappatoire sas de décompression

:::spoiler[la position des objets et éléments importants]
![Image interaction 1](/jeux/vlr/guide/echappatoires/sas_de_decompression/interaction_1.webp)

![Image interaction 2](/jeux/vlr/guide/echappatoires/sas_de_decompression/interaction_2.webp)

:::

## Récupération des objets

Récupérez les objets suivants :
- Le document sur la porte du sas "extérieur" sur le banc à l’étage.
- Des combinaisons dans des casiers de l’étage.

## Résolution des énigmes

Après avoir enfilé les combinaisons, retournez au niveau du sas et ouvrez la porte. Le document que vous avez récupéré indique quelle couleur doit être relié avec quelle autre couleur. Il faut prendre en compte que les traits de liaison ne peuvent pas se croiser.

:::spoiler[la solution pour rentrer dans le sas]
![schéma porte pour rentrer](/jeux/vlr/guide/echappatoires/sas_de_decompression/porte_entree.webp)
:::

Une fois rentré, regardez l’autre porte du sas pour que Yotsuba décide de partir, et suite à cela la décompression va démarrer.

Prenez l’écran portable sur le banc.

Retournez-vous et interagissez avec l’énorme Zéro Lapin pour le faire éclater. Une peluche Zéro Lapin était à l’intérieur : prenez-la.

Au niveau des tuyaux, vous pouvez enlever les plaques métalliques entre chaque section. Cela fera apparaitre d’autres tuyaux ainsi que des petits modules jaunes et violets qui sont en fait des lasers. Il y en a 2 de chaque, ramassez-les tous.

Il faut les remettre de manière à ce que le flux rouge aille jusqu’à la console avec les boutons.

:::spoiler[comment placer les lasers]
C’est uniquement sur la partie gauche qu’il faut mettre les lasers.
![placement lasers](/jeux/vlr/guide/echappatoires/sas_de_decompression/tuyaux.webp)
:::

Vous pouvez maintenant appuyer sur le bouton rouge de la machine pour arrêter la décompression. Au passage, prenez la petite clé en dessous du bouton rouge.

Avant de sortir, prenez note des chiffres sur le banc.

Regardez à travers la vitre pour communiquer avec Yotsuba. Sélectionnez le bas de la machine avec les compteurs pour récupérer le document sur la porte du sas intérieur et servez-vous-en pour rouvrir la porte.

:::spoiler[la solution pour sortir dans le sas]
![schéma porte pour rentrer](/jeux/vlr/guide/echappatoires/sas_de_decompression/porte_sortie.webp)
:::

La clé sert à ouvrir le casier de droite, dans lequel vous trouverez un papier avec des diagrammes colorés.

Remontez à l’étage pour enlever les combinaisons et regardez les formes sur le banc. En associant les formes ce banc avec les chiffres de l’autre banc, vous devriez pouvoir trouver le code pour récupérer la clé dans l’ascenseur.

:::spoiler[le code pour la clé de l’ascenseur]
![code pour la clé de l’ascenseur](/jeux/vlr/guide/echappatoires/sas_de_decompression/code_cle.webp)
:::

Redescendez pour ouvrir le casier du milieu avec la clé. Dedans se trouve la petite combinaison. Combinez-la avec la peluche de Zéro Lapin.

Remontez. Dans la sorte de photomaton, branchez la tablette au câble et placez Zéro Lapin en combinaison sur le tabouret.

Vous pouvez maintenant lancer le mini-jeu sur la tablette. Sur la note que vous avez trouvée, il y a des suites de couleurs. chaque ligne va correspondre à un chiffre. Quand vous cliquez sur un segment, cela va correspondre à une couleur. Pour obtenir le chiffre d’une ligne il faut comprendre quel chiffre les segments des couleurs de la ligne forment.

:::spoiler[quel nombre est à rentrer]
623
:::

Vous pouvez sortir de la salle pour terminer l’échappatoire.

## Obtenir le 2nd mot de passe du coffre

Au lieu de vous baser sur les couleurs de la note, regardez plutôt le carré de couleur sur la machine, entre la tablette et le tabouret. C’est la même logique qu’avec la note pour trouver les chiffres.

:::spoiler[quel nombre est à rentrer]
117
:::