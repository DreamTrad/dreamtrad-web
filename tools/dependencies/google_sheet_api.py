...
def set_credentials_info(credentials_info: dict[str, Any], reload: bool = False) -> None:
    ...
    if gc is None or reload:
        try:
            ...
            print("Google Sheets credentials set successfully.")
        except ValueError as e:
            print("Credentials not correctly set in config: %s", e)
    else:
        print("Credentials already set, skipping.")
...
def _safe_execute_method(obj: Any, method_name: str, *args: Any, **kwargs: Any) -> Any | Exception:
    ...
    print("Trying method: %s on %s", method_name, obj)
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            if "429" in str(e) or "500" in str(e) or "503" in str(e):
                print("Waiting for token, retry in %s seconds, error: %s", WAIT_TIME, e)
    ...
    print("After %s retries, failed to execute %s.", MAX_RETRIES, method_name)
...
def open_spreadsheet(sheet_id: str) -> Spreadsheet | None:
    ...
    if isinstance(spreadsheet, Exception):
        if "MAX_RETRIES" not in str(spreadsheet):
            print("Spreadsheet %s can't be opened, error: %s", sheet_id, spreadsheet)
        return
    return spreadsheet
...
def open_worksheet(spreadsheet: Spreadsheet, sheet_index: int) -> Worksheet | None:
    ...
    if isinstance(worksheet, Exception):
        if "MAX_RETRIES" not in str(worksheet):
            print("Worksheet %s of spreadsheet name:%s, id:%s can't be opened, error: %s",
...
def get_worksheet_values(worksheet: Worksheet) -> (list[list[str]] | None):
    ...
    if isinstance(values, Exception):
        if "MAX_RETRIES" not in str(worksheet):
            print("Can't get values of worksheet name:%s, id:%s", worksheet.title, worksheet.id)
        return
    return values
